...
def generate_smooth_data(
    ...
    for t in range(1, num_steps + burn_in):
     ...
     #    print("k_E_t is equaaaaal tooooo", k_E_t)
        ...
        k_E_t = k_E_t * (weathermob_data[t - 1][0]) * (abs(weathermob_data[t - 1][1]))
...
def get_data(data_cfg: dict, h5group: h5.Group) -> torch.Tensor:
    ...
    if "load_from_dir" in data_cfg.keys():
        ...
        import pandas as pd
        training_data = torch.from_numpy(pd.read_csv(data_cfg["load_from_dir"], index_col=0).to_numpy(dtype=float)).float().reshape(-1, 12, 1)
...
def get_weathermob_data(data_cfg: dict, h5group: h5.Group) -> torch.Tensor:
    """Returns the training data for the Covid model. If a directory is passed, the data is loaded from that directory.
    Otherwise, synthetic training data is generated by iteratively solving the ODE system.
    ...
    # Load weather-mobility data from file
    log.info("   Loading weathermob data ...")
    log.info("   Loading weathermob data ...")
    # Load weathermob data from hdf5 file
    import pandas as pd
    weathermob_data = torch.from_numpy(pd.read_csv(data_cfg["load_from_dir"], index_col=0).to_numpy(dtype=float)).float().reshape(-1, 2, 1)

    # Save weathermob data to hdf5 dataset and return
    dset_true_counts = h5group.create_dataset(
        "weathermob",
        weathermob_data.shape,
        maxshape=weathermob_data.shape,
        chunks=True,
        compression=3,
        dtype=float,
    )

    dset_true_counts.attrs["dim_names"] = ["time", "category", "dim_name__0"]
    dset_true_counts.attrs["coords_mode__time"] = "trivial"
    dset_true_counts.attrs["coords_mode__category"] = "values"
    dset_true_counts.attrs["coords__category"] = ["outFr", "D"]
    dset_true_counts.attrs["coords_mode__dim_name__0"] = "trivial"

    dset_true_counts[:, :, :] = weathermob_data

    return weathermob_data