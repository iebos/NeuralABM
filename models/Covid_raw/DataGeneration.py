...
#    k_q: float = cfg.get("k_q", 10.25)
    ...
    if init_state is None:
        init_state = torch.zeros(4, 1, dtype=torch.float)
        init_state[
    ...
    data = torch.empty((num_steps + burn_in, 4, 1), dtype=torch.float)
    data[0, :] = init_state
    ...
    for t in range(1, num_steps + burn_in):
        ...
        dy = torch.stack(
            [
                (-k_E_t * data[t - 1][Compartments.infected.value])
                * data[t - 1][Compartments.susceptible.value],
                k_E_t
                * data[t - 1][Compartments.susceptible.value]
                * data[t - 1][Compartments.infected.value]
                - k_R_t * data[t - 1][Compartments.infected.value]
                - k_D_t * data[t - 1][Compartments.infected.value],
                k_R_t * data[t - 1][Compartments.infected.value],
                k_D_t * data[t - 1][Compartments.infected.value],
            ]
        )
