...
#            k_Q = self.k_q * parameters["k_CT"] * densities[-1][-1]
            ...
            densities.append(
                torch.clip(
                    ...
                    + torch.stack(
                        [
                            -parameters["k_E"] * densities[-1][1],
                            parameters["k_E"] * densities[-1][0]
                            - parameters["k_R"] * densities[-1][1]
                            - parameters["k_D"] * densities[-1][1],
                            parameters["k_R"] * densities[-1][1],
                            parameters["k_D"] * densities[-1][1],
                        ]
                    )
                ...
                torch.concat(
                    [
                        torch.sum(
                            torch.pow(
                                densities[:, 0:2]
                                - self.true_data[start : start + self.batch_size, 0:2],
                                2,
                            ),
                            dim=0,
                        ),
                        torch.sum(
                            torch.pow(
                                torch.sum(densities[:, 2:3], dim=1)
                                - self.true_data[start : start + self.batch_size, 2:3],
                                2,
                            ),
                            dim=0,
                            keepdim=True,
