...
Covid = import_module_from_path(mod_path=up(up(__file__)), mod_str="Covid_raw_allcomps")
...
class Covid_NN:
    ...
    def epoch(self):
        ...
        for batch_no, batch_idx in enumerate(self.batches[:-1]):
            ...
            # Initialize current_densities with zeros
            # current_densities = torch.zeros(12, dtype=torch.float, requires_grad=True)
            # # Clone the training data to avoid in-place operations
            # training_data_clone = self.training_data[batch_idx].clone().detach().requires_grad_(True)
            # # Assign values from training_data to the appropriate positions in current_densities
            # current_densities[0] = training_data_clone[0]  # susceptible
            # current_densities[3] = training_data_clone[2]  # exposed
            # current_densities[4] = training_data_clone[1]  # infected
            # current_densities[7] = training_data_clone[3]  # recovered
            ...
            tensors_to_concat = [
                training_data_clone[0].unsqueeze(0),  # susceptible
                torch.zeros(1, dtype=torch.float, requires_grad=True).unsqueeze(0), # exposed
                torch.zeros(1, dtype=torch.float, requires_grad=True).unsqueeze(0),  # infected
                training_data_clone[2].unsqueeze(0),  # recovered
                training_data_clone[1].unsqueeze(0),  # symptomatic
                torch.zeros(1, dtype=torch.float, requires_grad=True).unsqueeze(0),  # hospitalized
                torch.zeros(1, dtype=torch.float, requires_grad=True).unsqueeze(0),  # critical
                training_data_clone[3].unsqueeze(0),  # deceased
                torch.zeros(1, dtype=torch.float, requires_grad=True).unsqueeze(0),  # quarantine_S
                torch.zeros(1, dtype=torch.float, requires_grad=True).unsqueeze(0),  # quarantine_E
                torch.zeros(1, dtype=torch.float, requires_grad=True).unsqueeze(0),  # quarantine_I
                torch.zeros(1, dtype=torch.float, requires_grad=True).unsqueeze(0),  # contact_traced
            ]

            # Concatenate the tensors to form current_densities
            current_densities = torch.cat(tensors_to_concat)
            densities = [current_densities]
            # Initialize the observed compartments: susceptible (0), exposed (1), recovered (2), and deceased (3)
            ...
            if self.Berlin_data_loss:
                ...
                if self.Berlin_data_loss:
                    # Select only the compartments susceptible (0), symptomatic (4), recovered (3), and deceased (7)
                    selected_densities = densities[:, [0, 4, 3, 7]]
                    selected_training_data = self.training_data[batch_idx + 1: self.batches[batch_no + 1] + 1,
                                             [0, 1, 3]]
